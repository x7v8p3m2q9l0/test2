...
local function write_log(msg_bits)
    ...
    if check_out_of_space() then
        -- [PATCH] rotate to .1 instead of deleting outright, so a post-mortem after a crash
        -- still has recent history available instead of starting from a blank file
        _log.file.close()

        local rotated = _log.path .. ".1"
        if fs.exists(rotated) then fs.delete(rotated) end

        -- only rotate if there's enough free space to keep both; otherwise fall back to delete
        if fs.getFreeSpace(_log.path) > MIN_SPACE then
            fs.move(_log.path, rotated)
        else
            fs.delete(_log.path)
        end
        ...
        _log.file.writeLine(time_stamp .. WRN_TAG .. "recycled log file (previous log preserved at " .. rotated .. " if space allowed)")
        _log.file.writeLine(stamped)
        _log.file.flush()
