...
function network.init_mac(passkey)
    ...
    -- [PATCH] raised from 256 -> 1000 iterations. This is still well under modern
    -- recommendations (OWASP suggests 600k+ for SHA1-PBKDF2) but CC:Tweaked's compute
    -- budget makes very high counts impractical for a one-time boot-time cost; 1000
    -- meaningfully raises brute-force cost over the previous value with negligible
    -- impact on init time.
    key_deriv.setIterations(1000)
    -- KNOWN LIMITATION: this salt is a fixed constant, not unique per facility. Two
    -- facilities using the same passkey derive an identical HMAC key. A proper fix
    -- requires a per-facility random salt stored in config and exchanged/verified
    -- during ESTABLISH, which is a larger change across every component's config
    -- schema - out of scope for this patch, flagged here for future work.
...
---@diagnostic disable-next-line: need-check-nil
    ...
    function public.connect(reconnected_modem)
        ...
        -- [PATCH] authenticate whenever a passkey is configured, regardless of modem type
        -- wired modems are trivially tapped on CC cable networks, so HMAC must apply to both
        self.use_hash = _crypt.hmac ~= nil
        self.phy_up   = true
        self.link_up  = false
