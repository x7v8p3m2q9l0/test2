...
function update.auto_control(ExtChargeIdling)
        ...
        elseif self.last_update < charge_update then
            ...
            if open_loop then
                ...
                if not self.saturated then
                    ...
                    -- [PATCH] also clamp the upper bound: without this, the integral term can wind up
                    -- indefinitely while charge is climbing toward setpoint (error stays positive but small),
                    -- causing overshoot once conditions change. Cap so I-term alone can't exceed max burn.
                    local i_cap = math.abs(self.max_burn_combined / math.max(CHARGE_Ki, 1e-6))
                    self.accumulator = math.max(0, math.min(i_cap, self.accumulator + (error * (now - self.last_time))))
                end
    ...
    elseif self.mode == PROCESS.GEN_RATE then
        ...
        elseif self.last_update < rate_update then
            ...
            -- stop accumulator when saturated to avoid windup
            if not self.saturated then
                -- [PATCH] clamp both directions; gen-rate error can be positive or negative
                -- (unlike charge mode) so an unclamped accumulator can wind up in either direction
                -- during sustained turbine ramp transients.
                local i_cap = math.abs(self.max_burn_combined / math.max(RATE_Ki, 1e-6))
                self.accumulator = math.max(-i_cap, math.min(i_cap, self.accumulator + (error * (now - self.last_time))))
            end
